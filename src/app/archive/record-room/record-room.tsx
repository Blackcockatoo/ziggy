...
export const recordRoomTracks: Track[] = [
  {
    no: 1,
    title: "Good Morning, Frankston",
    assetSlug: "01-good-morning-frankston",
  ...
  },
  {
    ...
    assetSlug: "02-the-old-vic-state",
  ...
  },
  {
    ...
    assetSlug: "04-there-is-another-procedure",
  ...
  },
  {
    ...
    assetSlug: "10-who-controls-the-controller",
  ...
  },
  {
    ...
    assetSlug: "13-the-old-vic-state-endures",
...
function coverSrc(track: Track) {
  return `/archive/record-room/covers/${track.assetSlug}.webp`;
}

function audioSrc(track: Track) {
  return `/archive/record-room/audio/${track.assetSlug}.mp3`;
}
...
export function RecordRoom() {
  ...
  const active = useMemo(
    () => recordRoomTracks[current] ?? recordRoomTracks[0],
    [current],
  );
  ...
  const moveTrack = async (offset: -1 | 1) => {
    const nextIndex = Math.max(
      0,
      Math.min(recordRoomTracks.length - 1, current + offset),
    );
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.grid} aria-label="ZIGGIE track sleeves">
        {recordRoomTracks.map((track, index) => {
      ...
      <div className={styles.player}>
        ...
        <a
          href={audioSrc(active)}
          target="_blank"
          rel="noreferrer"
          className={styles.openAudio}
          aria-label={`Open ${active.title} audio in a new tab`}
        >
          OPEN
        </a>
        <button
          ...
          disabled={current === recordRoomTracks.length - 1}
        >
          →
        </button>
      </div>
